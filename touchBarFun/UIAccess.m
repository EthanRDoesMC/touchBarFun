...
long getLongAttribute(AXUIElementRef element, CFStringRef attribute) {
    CFNumberRef valueRef = (__bridge CFNumberRef)(getAttribute(element, attribute));
    long result = 0;
    if (valueRef) {
...
NSArray *menuItemsForElement(AXUIElementRef element, NSInteger depth, NSString *elementName, NSInteger maxDepth, NSDictionary *virtualKeys) {
    ...
    AXUIElementCopyAttributeValue(element, kAXChildrenAttribute, (void *)&children);
    ...
    for (id child in children) {
        ...
        NSString *name = getMenuItemTitle((__bridge AXUIElementRef) child);
...
//        }
        ...
        BOOL enabled = isEnabled((__bridge AXUIElementRef) child);


        NSArray *mActions = nil;
        AXUIElementCopyActionNames(element, (CFTypeRef)&mActions);

        MenuItem *menuItem = [[MenuItem alloc] initWithRef:(__bridge AXUIElementRef) child];
        //menuItem.element = (AXUIElementRef)&element;
        menuItem.enabled = enabled;
        menuItem.name = name;
        menuItem.depth = depth;
        menuItem.actions = mActions;
        menuItem.shortcut = getMenuItemShortcut((__bridge AXUIElementRef) child, virtualKeys);

        if (mChildrenCount > 0 || mChildrenCount < 40 || depth < maxDepth) {
            menuItem.children = menuItemsForElement((__bridge AXUIElementRef) child, depth + 1, name, maxDepth, virtualKeys);
        }
