...
//    [NSTouchBar presentSystemModalTouchBar:[[SubmenuTouchBar alloc] initWithMenu:button.item.submenu] placement:1  systemTrayItemIdentifier:button.item.submenu.title];
    ...
    [subMenuBar presentAsSystemModalForItemIdentifier:button.externalItem.name placement:TRUE];
}
...
-(void)presentTouchBar {
    [self.touchBar presentAsSystemModalForItemIdentifier:@"com.ethanrdoesmc.touchbarfun" placement:TRUE];
}
- (void)windowDidLoad {
    ...
    _menuBarItem.view = [NSButton buttonWithImage:[NSImage imageWithSystemSymbolName:@"chevron.left" accessibilityDescription:nil] target:self action:@selector(presentTouchBar)];
    [NSTouchBarItem addSystemTrayItem:_menuBarItem];
    [_menuBarItem setControlStripPresence:YES];
